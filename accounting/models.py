...
class Policy(db.Model):
    ...
    policy_number = db.Column(
        u'policy_number', db.VARCHAR(length=128), nullable=False)
    effective_date = db.Column(u'effective_date', db.DATE(), nullable=False)
    status = db.Column(u'status', db.Enum(
        u'Active', u'Canceled', u'Expired'), default=u'Active', nullable=False)
    billing_schedule = db.Column(u'billing_schedule', db.Enum(
        u'Annual', u'Two-Pay', u'Quarterly', u'Monthly'), default=u'Annual', nullable=False)
    ...
    invoices = db.relation(
        'Invoice', primaryjoin="Invoice.policy_id==Policy.id")
...
class Contact(db.Model):
    ...
    role = db.Column(u'role', db.Enum(
        u'Named Insured', u'Agent'), nullable=False)
...
class Invoice(db.Model):
    ...
    # column definitions
    id = db.Column(u'id', db.INTEGER(), primary_key=True, nullable=False)
    policy_id = db.Column(u'policy_id', db.INTEGER(),
    ...
    deleted = db.Column(u'deleted', db.Boolean, default=False,
...
class Payment(db.Model):
    ...
    policy_id = db.Column(u'policy_id', db.INTEGER(),
                          db.ForeignKey('policies.id'), nullable=False)
    contact_id = db.Column(u'contact_id', db.INTEGER(),
                           db.ForeignKey('contacts.id'), nullable=False)
    amount_paid = db.Column(u'amount_paid', db.INTEGER(), nullable=False)
    transaction_date = db.Column(
        u'transaction_date', db.DATE(), nullable=False)
...
class CancelPolicy(db.Model):
    ...
    cancel_reason = db.Column(
        u' cancel_reason', db.String(150), nullable=False)
